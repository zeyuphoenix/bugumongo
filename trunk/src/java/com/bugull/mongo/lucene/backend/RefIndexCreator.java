 ...
 */
public class RefIndexCreator extends ObjectIndexCreator {
    
    private final static Logger logger = Logger.getLogger(RefIndexCreator.class);
    
    private Class<?> refBy;
    
    public RefIndexCreator(Class<?> refBy, Object obj, String prefix){
        super(obj, prefix);
        this.refBy = refBy;
    }
            ...
            if(irb != null){
                ...
                for(int i=0; i<len; i++){
                    if(cls[i].equals(refBy)){
                        ...
                        try{
                            processProperty(doc, f, analyze, store, boost);
                        }catch(Exception e){
